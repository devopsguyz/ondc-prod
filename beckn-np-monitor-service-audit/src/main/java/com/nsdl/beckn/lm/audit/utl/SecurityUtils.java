package com.nsdl.beckn.lm.audit.utl;
...
import com.nsdl.beckn.lm.audit.model.CommonModel;
...
public class SecurityUtils implements ApplicationContextAware {
	...
	public CommonModel initCommonPropertiesWithId(CommonModel cm, UUID id, String ip) {
		...
		if (cm.getVersion() == null) {
			...
			cm.setVersion(1);
			cm.setCreatedDate(OffsetDateTime.now(ZoneOffset.UTC));
			cm.setSourceIp(ip);
		} else {
			cm.setUpdatedBy(id);
			cm.setUpdatedDate(OffsetDateTime.now(ZoneOffset.UTC));
