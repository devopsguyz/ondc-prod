...
@Repository
public interface ApiAuditMumbaiArchalRepository extends ApiAuditParentRepository<ApiAuditMumbaiArchalEntity> {
	/**
	 ...
	 */
	@Query(value ="select transaction_id from api_audit_archvl apiAuditEntity where apiAuditEntity.created_on between ?1 and ?2 ", nativeQuery = true)
	...
	@Query( value = " select "
			...
			+ "        api_audit_archvl  where created_on between ?1 and ?2 "
			+ "    order by "
	...
	/**
	 * Gets transaction distinct id from api audit by transaction id. 
	 */
	@Query(value = " select "
	...
	/**
	 * Select seller transaction details where seller id is not null and type = response by seller from
	 * ApiAuditEntitySellerProjection by start and end local date time.
	 */
	@Query( value = " Select  seller_id sellerId, count(1) from api_audit_archvl where seller_id is not null and "
	...
	/**
	 * Select seller details where seller is not null and type = response to buyer from api audit by start and end local date time.
	 */
	@Query( value = " Select  buyer_id buyerId, count(1) from api_audit_archvl where seller_id is not null and "
