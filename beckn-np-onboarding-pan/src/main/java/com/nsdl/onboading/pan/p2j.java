...
public class p2j {
	...
	public static void createSig(String certificateFile, String pwd, String jksFile) throws Exception {

		File fileIn = new File(certificateFile);

		File fileOut = new File(jksFile);
		if (!fileOut.exists()) {
			if (!fileIn.canRead()) {
				log.info("Unable to access input keystore: " + fileIn.getPath());
				System.exit(2);
			}
			...
			char inphrase[] = pwd.toCharArray();
			char outphrase[] = pwd.toCharArray();
			...
			out.close();
			log.info("Java Key Store created successfully");
		}
	}

	public static void main(String args[]) throws Exception {
		args = new String[] { "XtraTrustSubCA2022.pfx", "123456", "output.jks" };
		if (args.length < 1) {
